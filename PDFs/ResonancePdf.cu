...
EXEC_TARGET inline fptype hFun (const double &s, const double &daug2Mass, const double &daug3Mass) {
  ...
  double val = ((2/M_PI) * (k_s/SQRTs) * log( (SQRTs + 2*k_s)/(sm)));

  return val;
}

EXEC_TARGET inline fptype dh_dsFun (const double &s, const double &daug2Mass, const double &daug3Mass) {
  // Yet another helper function
  //const fptype _pi = 3.14159265359;
  double k_s = twoBodyCMmom(s, daug2Mass, daug3Mass);
  
  double val = (hFun(s, daug2Mass, daug3Mass) * (1.0/(8.0*pow(k_s, 2)) - 1.0/(2.0 * s)) + 1.0/(2.0* M_PI *s));
  return val;
}


EXEC_TARGET inline fptype dFun (const double &s, const double &daug2Mass, const double &daug3Mass) {
  ...
  double val = 3.0/M_PI * sm24/pow(k_m2, 2) * log((m + 2*k_m2)/sm) + m/(2*M_PI*k_m2) - sm24*m/(M_PI * pow(k_m2, 3));
  return val;
}

EXEC_TARGET inline fptype fsFun (double s, double m2, double gam, double daug2Mass, double daug3Mass) {
